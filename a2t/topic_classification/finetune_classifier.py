...
def parse_args():
    ...
    parser = argparse.ArgumentParser('finetune_classifier', 
...
def train(opt):
    ...
    eval_dataset = tokenizer(eval_glosses, padding=True, truncation=True, max_length=config['max_length'])
    ...
    for epoch in range(config['epochs']):
        ...
        correct, total = 0, 0
        progress = tqdm(enumerate(train_dataloader), desc=f"Epoch: {epoch} - Loss: {total_loss} - Accuracy: {accuracy}", 
                        total=len(train_dataset)//config['batch_size'])
        for i, batch in progress:
            # Batch to cuda
            input_ids, attention_mask, labels = batch

            loss, logits = model(input_ids=input_ids.cuda(),
        ...
        with torch.no_grad():
            total_loss, accuracy =  .0, .0
            correct, total = 0, 0
            progress = tqdm(enumerate(eval_dataloader), desc=f"Epoch: {epoch} - Loss: {total_loss} - Accuracy: {accuracy}", 
