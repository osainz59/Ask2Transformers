...
def generate_positive_example(instance: SlotFeatures, templates: Dict[str, list], 
                              type2role: Dict[str, list], n: int = 1, label2id=None):
    ...
    return [
        MNLIInputFeatures(
            ...
            hypothesis=template.format(
                    trg=instance.trigger, obj=instance.arg, trg_subtype=instance.trigger_type.split('.')[1]
            ),
...
def generate_neutral_example(instance: SlotFeatures, templates: Dict[str, list], 
                             type2role: Dict[str, list], n: int = 1, label2id=None):
...
def generate_negative_example(instance: SlotFeatures, templates: Dict[str, list], 
                              type2role: Dict[str, list], n: int = 1, label2id=None):
...
def to_nli(instance: SlotFeatures, templates: Dict[str, list], type2role: Dict[str, list],
           negn: int = 1, posn: int = 1, label2id=None):
    nli_examples = [
...
def main(opt):
    ...
    for instance in dataset:
        nli_examples.extend(
            to_nli(instance, config['template_mapping'], 
                   type2role, opt.negn, opt.posn, label2id=label2id)
        )
