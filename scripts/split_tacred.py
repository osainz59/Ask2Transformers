...
parser.add_argument('--splits', type=list, default=[0.01, 0.05, 0.1, 0.25, 0.5], nargs='+')
...
for split in sorted(args.splits):
    test_size = (1. / total)*(split - prev_split)
    total -= test_size
    prev_split = split
    ids, test_ids, labels, _ = train_test_split(ids, labels, test_size=test_size, stratify=labels)
...
for name, split in zip(args.splits, splits):
    ...
    with open(os.path.join(args.output_folder, partition + str(name)+'.split.txt'), 'wt') as f:
        for line in split:
            f.write(line+'\n')
    with open(os.path.join(args.output_folder, partition + str(name)+'.json'), 'wt') as f:
