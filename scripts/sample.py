...
parser.add_argument(
    "--output_file", type=str, default=".ignore/subsample.annotations.tsv.gz"
)
    ...
    with open_file(label_file, "rt") as f:
        for line in f:
            ...
            label = (
                "\t".join(line[1:3])
                if len(line) > 2
                else "\t".join(line[1:3]) + "\t1.0"
            )
            labels_dict[idx] = label
    all_labels.append(labels_dict)

with gzip.open(args.glosses, "rt", encoding="utf-8") as f, gzip.open(
    ...
    for line in f:
        ...
        output_row = (
            [idx]
            + [labels_dict.get(idx, "NULL\t0.0") for labels_dict in all_labels]
            + [gloss]
        )
